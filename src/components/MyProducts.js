...
function MyProducts() {
  ...
  async function displayRazorpay() {
    const totalCost = addCost.reduce(
      (accumulator, currentCost) =>
        parseInt(accumulator) + parseInt(currentCost),
      0
    );
  ...
  return (
    <div>
      <section className="d-flex d-col featured-products">
        <div className="row">
          <Link to={userInfo.role === "Customer" ? "/Home" : "/AddProduct"}>
            <button className="button-feature mb-3">Add Product</button>
          </Link>
          {userInfo.role === "Customer" ? (
            <button
              className="button-feature mb-3"
              onClick={() => {
                displayRazorpay();
                deleteAll();
              }}
